pipeline {
  ...
  environment {
        PROJECT_NAME = "bte-schema-web-react"
        DOCKER_REPO_NAME = "ncatsctsa/biothings_kg_ui"
    }
    stages {
        stage('Build Version') {
            when {
        ...
        stage('deploy docker') {
            ...
            steps {
                ...
                checkout scm
                configFileProvider([
                    configFile(fileId: 'bte-schema-dev-docker-compose', targetLocation: 'docker-compose.yml')     
                ]) {    
                   script {
                        sh '''
